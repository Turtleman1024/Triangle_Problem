 ...
 * CptS 422 Homework 1
 * 
 * Purpose: This class holds the representation of a test to check side a's boundary values of the triangle.
 */
package testpackage;
...
class TestCheckRangeA {
	...
	@Test
	void testCheckRangeA() throws IOException {
		...
		pw.println("-------Test Suite Check Range A------");
		...
		triangleUnderTest.setB(testValues[3]);
		triangleUnderTest.setC(testValues[3]);
		
		for(int index = 0; index < testValues.length; index++)
		{
			
			triangleUnderTest.setA(testValues[index]);
			
			if(index == 0 || index == 6)
			{
				pw.print("A Out of bounds \t");
				...
				pw.print(output + "\t");
				assertEquals(output, testOutputs[0]);
				pw.println("Passed");
			}
			else
			{
				pw.print("A In the bounds \t");
				pw.print(triangleUnderTest.getA() + "\t\t");
				pw.print(triangleUnderTest.getB() + "\t\t");
				pw.print(triangleUnderTest.getC() + "\t\t");
				output = triangleUnderTest.checkRange();
		...
		pw.println("-----End Test Suite Check Range A-----");
		pw.println(" ");
