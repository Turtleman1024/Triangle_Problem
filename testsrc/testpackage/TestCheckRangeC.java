 ...
 * CptS 422 Homework 1
 * 
 * Purpose: This class holds the representation of a test to check side c's boundary values of the triangle.
 */
package testpackage;
...
class TestCheckRangeC {
	...
	@Test
	void testCheckRangeC() throws IOException {
		...
		pw.println("-------Test Suite Check Range C------");
		...
		{
			triangleUnderTest.setC(testValues[index]);
			
			if(index == 0 || index == 6)
			{
				pw.print("C Out of bounds \t");
				...
				pw.print(output + "\t");
				assertEquals(output, testOutputs[2]);
				pw.println("Passed");
			}
			else
			{
				pw.print("C In the bounds \t");
				pw.print(triangleUnderTest.getA() + "\t\t");
				pw.print(triangleUnderTest.getB() + "\t\t");
				pw.print(triangleUnderTest.getC() + "\t\t");
				output = triangleUnderTest.checkRange();
		...
		pw.println();
		pw.println("-----End Test Suite Check Range C-----");
		pw.println(" ");
